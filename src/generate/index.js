...
export async function processRoutes() {
	// update cache
	Object.values(config.PROPS).forEach((update) => (config.CACHE[update.route] = update));
...
function buildTree(entry, paths, file, isRoot = false) {
	...
	if (!next) {
		node.push({ id: targetDirectory, label: config.ORIGINALS[targetDirectory], children: [] });
